...
def _default_rc_file_creation_step():
    ...
    ans_yes = 'y' # prompt_y_n('Could not automatically find a suitable file to use. Create {} now?'.format(rcfile), default='y')
    if ans_yes:
        open(rcfile, 'a').close()
        return rcfile
    return None
...
def get_rc_file_path():
    ...
    rc_file = rc_file or default_rc_file # prompt_input_with_default('Enter a path to an rc file to update', default_rc_file)
    if rc_file:
...
def handle_path_and_tab_completion(completion_file_path, exec_filepath, exec_dir):
    ans_yes = 'y' # prompt_y_n('Modify profile to update your $PATH and enable shell/tab completion now?', 'y')
    if ans_yes:
...
def main():
    ...
    install_dir = DEFAULT_INSTALL_DIR # get_install_dir()
    exec_dir = DEFAULT_EXEC_DIR # get_exec_dir()
